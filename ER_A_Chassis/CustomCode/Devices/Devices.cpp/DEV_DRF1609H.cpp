...
{
    if(TJC4827X343_Data[0] == 0x53 && TJC4827X343_Data[4]==0x45 && TJC4827X343_Data[1]==0x02)
    { 
		if(Deal_Flag != 0)
		{
...
//	{
			...
			buffer[len+1] = 0xff;
			buffer[len+2] = 0xff;
			HAL_UART_Transmit_IT(&huart7, (uint8_t *)buffer, len+3);
//	}
...
void DRF1609H_classdef::communication(void)
{
	if(time_num > 0)
	{
		...
		{
			case 0x8101:
				TJCPrintf("x0.val=%d",(int)Auto.Posture.POS_X());
				TJCPrintf("y0.val=%d",(int)Auto.Posture.POS_Y());
				TJCPrintf("w0.val=%d",(int)Auto.Posture.POS_W());
			break;
