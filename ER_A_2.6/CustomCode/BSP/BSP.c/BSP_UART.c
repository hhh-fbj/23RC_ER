...
void Uart_Init(UART_HandleTypeDef* huart, uint8_t* Rxbuffer, uint32_t length, usart_call_back fun)
{
	...
	{
			usart8_manage.rx_buffer = Rxbuffer;
			usart8_manage.rx_buffer_size = length;
			usart8_manage.uart_x = huart;
			usart8_manage.call_back_fun = fun;
			__HAL_UART_ENABLE_IT(huart,UART_IT_IDLE);
			__HAL_UART_CLEAR_IDLEFLAG(huart);
			HAL_UART_Receive_DMA(huart, Rxbuffer, length);
	}
...
{
    return HAL_UART_Transmit_DMA(usart8_manage.uart_x, msg, len);
}
