...
/**
  ...
  * @retval         0:校准任务还没有完
                    1:校准任务已经完成
  */
static uint8_t cali_gyro_hook(uint32_t *cali, uint8_t cmd);   //gyro device cali function
...
/**
  ...
  * @param[in]      none
  * @retval         none
  * @note			放到INS_Task里调用 不单独再开一个任务
  */
void IMU_Cali(uint8_t key_lever)
{
	static uint8_t key_last_level = key_up; //--- 初始化为未按下

	if((key_lever == key_down && key_last_level == key_up))
	{
		cali_sensor[0].cali_cmd = 1;
	}
	//保存上一时刻key的电平
	key_last_level = key_lever;
				
	//若开启校准
	if (cali_sensor[0].cali_cmd)
	{		
		//并且校准的处理钩子函数不为空
		if (cali_sensor[0].cali_hook != NULL)
		{			
			//则通过实时的gyro数据进行计算得到gyro_offset实际用于计算的Angle的偏移值，
			//并将此漂移值赋给cail_sensor[i].buf中校准数据数组，可以用于写入flash中
			if (cali_sensor[0].cali_hook(cali_sensor_buf[0], CALI_FUNC_CMD_ON))
			{	
				//确定传感器的名字 和 校准状态将其 置为 已经校准
				//done
				cali_sensor[0].name[0] = cali_name[0][0];
				cali_sensor[0].name[1] = cali_name[0][1];
				cali_sensor[0].name[2] = cali_name[0][2];
				//set 0x55
				cali_sensor[0].cali_done = CALIED_FLAG;
				//并将开启校准的标准位清0 
				cali_sensor[0].cali_cmd = 0;
				//并将校准完的数据写入flash中供于下次开启可用
				//write
				cali_data_write();
			}
